...
--Proc Ignition Skill
function Auxiliary.TurboDuelStart(c,id)
...
function Auxiliary.TDStartUp(c,id)
	return function(e,tp,eg,ep,ev,re,r,rp)
		...
		eh:SetOperation(returnop)
		Duel.RegisterEffect(eh,0)
		local ek=Effect.GlobalEffect()
		ek:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_GRANT)
		ek:SetTargetRange(LOCATION_SZONE,LOCATION_SZONE)
		ek:SetProperty(EFFECT_FLAG_IGNORE_IMMUNE+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
		ek:SetLabelObject(e1)
		Duel.RegisterEffect(ek,0)
		--Speed Counter
...
function ctop(e,tp,eg,ep,ev,re,r,rp)
	Duel.RegisterFlagEffect(tp,2021020004)
	local n=Duel.GetFlagEffect(tp,202120004)
	if n>3 then n=3 end
	e:GetHandler():AddCounter(0x91,n)
end
