...
function s.initial_effect(c)
   ...
   local e1=Effect.CreateEffect(c)
	...
	se:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	se:SetCode(EVENT_PHASE+PHASE_DRAW)
	se:SetCondition(s.setop)
	Duel.RegisterEffect(se,0)
	local ge=Effect.CreateEffect(c)
	ge:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
...
function s.getop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetTurnCount()==1
		...
		if Duel.GetTurnPlayer()==1-tp then
			Duel.RegisterFlagEffect(tp,id+1,0,0,Duel.GetLP(tp))
		else
			Duel.RegisterFlagEffect(tp,id+1,0,0,Duel.GetLP(1-tp))
		end
	end
	--Player Two
	if Duel.GetTurnCount()==2
		...
		if Duel.GetTurnPlayer()==1-tp then
			Duel.RegisterFlagEffect(tp,id+2,0,0,Duel.GetLP(tp))
		else
			Duel.RegisterFlagEffect(tp,id+2,0,0,Duel.GetLP(1-tp))
		end
	end
	--Player Three
	if Duel.GetTurnCount()==3
		...
		if Duel.GetTurnPlayer()==1-tp then
			Duel.RegisterFlagEffect(tp,id+3,0,0,Duel.GetLP(tp))
		else
			Duel.RegisterFlagEffect(tp,id+3,0,0,Duel.GetLP(1-tp))
		end
	end
	--Player Four
	if Duel.GetTurnCount()==4
		...
		if Duel.GetTurnPlayer()==1-tp then
			Duel.RegisterFlagEffect(tp,id+4,0,0,Duel.GetLP(tp))
		else
			Duel.RegisterFlagEffect(tp,id+4,0,0,Duel.GetLP(1-tp))
		end
	end
end