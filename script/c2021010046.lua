--Number C48 Shadow Reaper (WIP)
local s,id=GetID()
function s.initial_effect(c)
	--xyz summon
	Xyz.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsAttribute,ATTRIBUTE_DARK),4,3,s.ovfilter,aux.Stringid(id,0))
	c:EnableReviveLimit()
	--token
...
function s.atkval(e,c)
	return Duel.GetMatchingGroup(Card.IsFaceup,tp,LOCATION_MZONE,LOCATION_MZONE,nil):GetMaxGroup(Card.GetAttack)
end
function s.con(e)
